...
import com.starlight.core.entity.bo.CalendarInfo;
...
public final class DateUtils {
    ...
    public static <T> List<CalendarInfo<T>> calender(int year, int month, final Function<LocalDate, Optional<T>> func){
        if (MathUtils.nvl(year) < 1111 || !MathUtils.isBetween(month, 1, 12)) {
            return Lists.newArrayList();
        }
        List<CalendarInfo<T>> cdList = Lists.newArrayList();
        int monthDays = DateUtils.monthDays(year, month);
        CalendarInfo<T> cdR;
        for(int day = 1; day < monthDays + 1; day++){
            cdR = new CalendarInfo<>();
            cdR.day = day;
            final LocalDate date = LocalDate.of(year, month, day);
            cdR.weekDay = DateUtils.dayOfWeek(date);
            cdR.isToday = DateUtils.isToday(date);
            if(null != func){
