...
public class RedisUtils {
    ...
    public static <T> T get(String key, Class<T> clz) {
        Object result = key == null ? null : getRedisTemplate().opsForValue().get(key);
    ...
    public static boolean set(String key, Object value, long time) {
        try {
            if (time > 0) {
                getRedisTemplate().opsForValue().set(key, value, time, TimeUnit.MILLISECONDS);
            } else {
                set(key, value);
            }
            return true;
    ...
    public static Long sSet(String key, String... value) {
        try {

            return getRedisTemplate().opsForSet().add(key, value);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static boolean lSet(String key, Integer index, Object value) {
        try {
    ...
    public static BoundHashOperations<String, Object, Object> getBoundHash(String key){
        BoundHashOperations<String, Object, Object> boundHashOperations = getRedisTemplate().boundHashOps(key);
        return boundHashOperations;
    }

    public static Set<String> scan(String keyHead) {
        Set<String> keys = getRedisTemplate().execute((RedisCallback<Set<String>>) connection -> {
            Set<String> keysTmp = new HashSet<>();
            Cursor<byte[]> cursor = connection.scan(new ScanOptions.ScanOptionsBuilder().match( keyHead + "*").count(1000).build());
