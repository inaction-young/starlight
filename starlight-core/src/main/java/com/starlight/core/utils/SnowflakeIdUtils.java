...
public final class SnowflakeIdUtils {
    ...
    public static String unique(){
        return String.valueOf(ID_WORKER.grantId());
    }

    public static long grant(){
        return ID_WORKER.grantId();
    }
    ...
    public static class Worker {
        ...
        // 获得下一个ID (该方法是线程安全的)
        public synchronized long grantId() {
        ...
        private int machineId(int bits) {
            if("hostname".equalsIgnoreCase(SpringUtils.getEnvironment().getProperty(SystemConstants.KEY_SNOW_MID_GENERATOR))) {
                return hostNameGenerator(bits);
            }
            return ipGenerator(bits);
        }
        private int hostNameGenerator(int customBits) {
            String hostName = NetworkUtils.localHostName();
        ...
        private int ipGenerator(int bits) {
            String ip = NetworkUtils.machineIP();
            if(StringUtils.isBlank(ip)) {
                return 0;
            }
            String binaryIp = Long.toBinaryString(NetworkUtils.ip2long(ip));
